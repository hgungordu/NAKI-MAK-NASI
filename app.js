...
function renderHistory(){const q=($("#historySearch")?.value||"").toLowerCase();$("#historyList").innerHTML=services.filter(s=>(s.no+" "+s.customer+" "+s.brand+" "+s.model).toLowerCase().includes(q)).map(serviceRow).join("")||'<div class="empty">Kayıt bulunamadı.</div>'}
